...
initial begin
    ...
    $readmemh("input.mem", input_image);
        ...
        for (xout = 0; xout < W_OUT; xout = xout + 1) begin
            ...
            output_image[yout * W_OUT + xout] =
                (1-a)*(1-b) * $itor(input_image[y0*W_IN + x0])
              + a    *(1-b) * $itor(input_image[y0*W_IN + x1])
              + (1-a)*b     * $itor(input_image[y1*W_IN + x0])
              + a    *b     * $itor(input_image[y1*W_IN + x1]);
        end
    end
